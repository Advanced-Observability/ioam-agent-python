...
def report_ioam(report_func, traces):
   try:
      for trace in traces:
         report_func(trace)
...
def parse_ioam_trace(data):
   try:
      nsId, node_len, _, remaining_len, trace_type = unpack(">u16u5u4u7u32", data[:8])
...
def listen(interface, output):
   try:
      ...
      if output == "outbound-ioam-trace":
         # retreive collector target IP from env variable
...
def main(script, argv):
   try:
      opts, args = getopt.getopt(argv, "hi:og", ["interface="])
